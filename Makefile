...
#    Updated: 2016/03/21 21:45:37 by amineau          ###   ########.fr        #
#                                                                              #
# **************************************************************************** #
...
SRCPRI = ft_printf.c ft_stock.c ft_list.c ft_algo.c ft_straddc.c ft_itoa_double.c\
...
SRCLIB = ft_putchar.c ft_putstr.c ft_putendl.c ft_putnbr.c ft_strlen.c ft_strdup.c \
		...
		get_next_line.c ft_abs.c ft_strcasecmp.c ft_strnchr.c ft_itoa_base.c ft_power.c \
		ft_strcln1join.c ft_itoa_unsi.c ft_itoa_unsi_base.c ft_strcln2join.c ft_atoi_base.c \

SRCDIR = srcs
OBJDIR = objs

SRC = $(addprefix $(SRCDIR)/,$(SRCLIB) $(SRCPRI))
OBJ = $(addprefix $(OBJDIR)/,$(SRCLIB:.c=.o) $(SRCPRI:.c=.o))
INC = includes
...
clean:
	...
	@printf "$(RED)"
	@rm -rf $(OBJDIR) 2>&1 | cut -d' ' -f3- \
	&& printf "$(GREENB)<<--$(WHITE)\n" \
	|| printf "$(WHITE)"

fclean: clean
	@printf "$(YELLOW)%-30s$(WHITE)" "Deleting $(NAME)"
	@rm $(NAME) 2>&1 | cut -d' ' -f3- >null
	@cmd="$(cat null)"
ifeq  ($(cmd), "")
		@printf "$(GREENB)<<--$(WHITE)\n"
else
		printf "$(RED)$(cmd)$(WHITE)"
endif
